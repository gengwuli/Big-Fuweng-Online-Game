...
//		pnlCtrl.add(btnShareMoney);
		...
		avatarlist.setCellRenderer(new AvatarRender<TAvatarListItem>());
...
//		btnShareMoney.setEnabled(isMyTurn);
	...
	private void rollDicePrompty() {
		JLabel label = new JLabel("Click OK to Roll the dice.", null, JLabel.CENTER);
		JOptionPane.showMessageDialog(null, label, "It's your turn!", JOptionPane.PLAIN_MESSAGE, image);
		rollDice();
	}
	
	private void shareMoneyPrompt() {
		int maxSharableMoney = Integer.parseInt(tfMoney.getText());
		if (maxSharableMoney==0) return;
		TAvatarListItem[] items = model.getTeammates();
		if (items==null || items.length==0) return;
		// show select teammate dialog
		JLabel teammatelabel = new JLabel("Would you like to share your money?", null, JLabel.CENTER);
		@SuppressWarnings("unchecked")
		TAvatarListItem item = (TAvatarListItem) JOptionPane.showInputDialog(null, teammatelabel, "Share Money", JOptionPane.PLAIN_MESSAGE, image, items, items[0]);
		if (item==null) return;
		// show select amount dialog
		List<Integer> sharableAmount = new ArrayList<>();
		for (int i=10; i<=maxSharableMoney; i+=10)
	...
	private void rollDice() {
		// steps
		int dice1 = randomno.nextInt(6)+1; // get steps (1-12)
		int dice2 = randomno.nextInt(6)+1;
		tfSteps.setText(Integer.toString(dice1+dice2)); // show steps in text field
		//Added by Gengwu 
		model.showDice(dice1, dice2);	
//		// move
//		model.moveAvatars(steps);
//		model.checkCityStatus();
	}
